  ...
  app.post(config.PA_MOCK.ROUTES.PPT_NODO, async (req, res) => {
    ...
    try {
      ...
      if (soapRequest[verifySoapRequest]) {
        ...
        const isFixOver = avviso13.test(noticenumber) || avviso18.test(noticenumber) || avviso19.test(noticenumber);
        ...
        if (!isValidNotice && !isExpiredNotice && !isTimeout) {
          ...
          if (b) {
            ...
            const paGetPaymentResponse = paGetPaymentRes({
              ...
              fiscalCodePA: fiscalcode,
              iban_1: iban1,
              iban_2: avviso19.test(noticenumber) ? iban1 : iban2,
              iban_3: iban3,
              iban_4: iban4,
              iban_5: iban5,
              outcome: 'OK',
